...
// Policy config with max_attempts: 2
const testPolicyConfig: PolicyConfig = {
  retry_limit: { max_attempts: 2 },
...
async function seedAllocatedEvent(suffix: string): Promise<string> {
  const result = await ingestEvent(repo, {
    source: "razorpay",
    external_ref: `chaos_retry_${suffix}`,
...
describe("Task 15: Retry-then-stop Chaos Test", () => {
  it("forces failure on attempts 1-2 and asserts the state machine reaches Stopped", async () => {
    const eventId = await seedAllocatedEvent("1");
    const event = (await repo.findEventById(eventId))!;

    // We want the gateway to fail for the first 2 attempts
    gateway.forceFailureFor(eventId, 2);

    // Attempt 1: Should fail and transition to Failed
    ...
    let state = await repo.getCurrentState(eventId);
    expect(state).toBe("Failed");

    // We mock that the worker pulled it again, and transition it back to Allocated so we can execute
    // Actually, executeDecision does `Allocated -> ActionSelected`. But if it's currently Failed,
    // does it throw? No, executeDecision directly calls `transition(Allocated, ActionSelected)` and
    // repo.updateState just inserts an audit_log with the new state! It doesn't check the current db state.
    
    // So we can just call executeDecision again for attempt 2.
    // Attempt 2: Should fail, but since max_attempts is 2, it should transition to Stopped.
    const outcome2 = await executeDecision(
      repo,
      llm,
      gateway,
      testPolicyConfig,
